...
{
  api.post('/subcon/SubConPayment',{sub_con_work_order_id:id})
  .then((res)=> {
  ...
  const SubCon = () => {
    ...
    Swal.fire({
     ...
     if (result.isConfirmed) {
       api.post('/subcon/SubConPayment',{sub_con_work_order_id:id}).then(res=>{
         console.log(res)
         Swal.fire(
           'Cancelled!',
   ...
   <ComponentCard>
        ...
        <tbody>
          {history && history.map(element=>{
              return (<tr key={element.sub_con_work_order_id}>
                <td>{moment(element.date).format('YYYY-MM-DD')}</td>
              <td>{element.amount}</td>
              <td>{element.mode_of_payment}</td>
              <td><Link to=""><span onClick={()=>SubCon(element.sub_con_work_order_id)}>Cancel</span></Link></td>            
                </tr>)
          })}
        </tbody>
        {/* <tfoot>
